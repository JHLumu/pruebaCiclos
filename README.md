...
| Camino básico | Nombre del caso | Entrada / Estado inicial | Valor/Estado esperado |
| :--- | :--- | :--- | :--- |
| **1 → 2 (Fin) → 7 → 8 → 9 (Fin) → 18 → 19 → Fin** | test\_Pendientes\_SinRealizados\_SinCursos | `listaCursos` es un objeto `ListaCursos` vacío. `realizados` es un `ArrayList` vacío. `tematica = "Informatica"`. | `maxNivel` se mantiene en 0. Ambos bucles se saltan. El `resultado` es una lista vacía. Se comprueba con `assertTrue(resultado.isEmpty())`. |
| **1 → 2 (Loop) → 3 (F) → 6 → 2 (Fin) → 7 → 8 → 9 (Fin) → 18 → 19 → Fin** | test\_Pendientes\_RealizadosOtraTematica\_SinCursos | `listaCursos` es un `ListaCursos` vacío. `realizados` contiene `[sociales1]` (temática "Sociales"). `tematica = "Informatica"`. | `maxNivel` se mantiene en 0 (el `IF` del 1er bucle es Falso). El 2º bucle se salta. El `resultado` es una lista vacía. Se comprueba con `assertTrue(resultado.isEmpty())`. |
| **1 → 2 (Loop) → 3 (T) → 4 → 5 → 8 → 9 (Fin) → 18 → 19 → Fin** | test\_Pendientes\_RealizadosTematicaOk\_SinCursos | `listaCursos` es un `ListaCursos` vacío. `realizados` contiene `[info1]` (temática "Informatica", Nivel 1). `tematica = "Informatica"`. | `maxNivel` se establece en 1 (el `IF` del 1er bucle es Cierto). El 2º bucle se salta. El `resultado` es una lista vacía. Se comprueba con `assertTrue(resultado.isEmpty())`. |
| **1 → 2 (Fin) → 7 → 8 → 9 (Loop) → 10,11 (F) → 17 → 9 (Fin) → 18 → 19 → Fin** | test\_Pendientes\_SinRealizados\_CursosOtraTematica | `listaCursos` es la lista global (con cursos de "Informatica" y "Sociales"). `realizados` es un `ArrayList` vacío. `tematica = "Salud"`. | `maxNivel = 0`. El 1er bucle se salta. El 2º bucle itera pero el `IF(10,11)` (coincidencia de temática) es siempre Falso. El `resultado` es una lista vacía. Se comprueba con `assertTrue(resultado.isEmpty())`. |
| **1 → 2 (Fin) → 7 → 8 → 9 (Loop) → 10,11 (T) → 12 (Fin) → 16 → 17 → 9 (Fin) → 18 → 19 → Fin** | test\_Pendientes\_SinRealizados\_SublistaVacia | `listaCursos` es un objeto que contiene una sublista vacía para "Informatica" (`listaTematicas=[ [] ]`). `realizados = []`. `tematica = "Informatica"`. | `maxNivel = 0`. El `IF(10,11)` es Cierto, pero el `FOR(12)` (bucle interno) no itera (0 elementos). El `resultado` es una lista vacía. Se comprueba con `assertTrue(resultado.isEmpty())`. |
| **1 → 2 (Loop) → 3 (T) → 4 → 5 → 8 → 9 (Loop) → 10,11 (T) → 12 (Loop) → 13 (F) → 15 → 12 (Fin) → 16 → 17 → 9 (Fin) → 18 → 19 → Fin** | test\_Pendientes\_RealizadosN1\_CursoN1 | `listaCursos` es una lista local que solo contiene `[info1]`. `realizados` contiene `[info1]`. `tematica = "Informatica"`. | `maxNivel = 1`. El `IF(13)` evalúa `(1 > 1)`, que es Falso. No se añaden cursos a `pendientes`. El `resultado` es una lista vacía. Se comprueba con `assertTrue(resultado.isEmpty())`. |
| **1 → 2 (Loop) → 3 (T) → 4 → 5 → 8 → 9 (Loop) → 10,11 (T) → 12 (Loop) → 13 (T) → 14 → 15 → 12 (Fin) → 16 → 17 → 9 (Fin) → 18 → 19 → Fin** | test\_Pendientes\_RealizadosN1\_CursoN2 | `listaCursos` es una lista local que solo contiene `[info2]`. `realizados` contiene `[info1]`. `tematica = "Informatica"`. | `maxNivel = 1`. El `IF(13)` evalúa `(2 > 1)`, que es Cierto. Se añade `info2` a `pendientes`. Se comprueba que el `resultado` tiene tamaño 1 y contiene `info2`. |
...
| Camino básico | Nombre del caso | Entrada / Estado inicial | Valor/Estado esperado |
| :--- | :--- | :--- | :--- |
| **1 → 2 (Fin) → 14 → 15 → Fin** | test\_MejorValorado\_ListaVacia | Se instancia un `ListaCursos` local y vacío (`listaVacia`). Se llama al método con `tematica="Info"` y `nivel=1`. | El bucle `for` (2) no itera (lista vacía). El método devuelve `null`. Se comprueba con `assertNull(resultado)`. |
| **1 → 2 (Loop) → 3,4 (F) → 13 → 2 (Fin) → 14 → 15 → Fin** | test\_MejorValorado\_TematicaNoEncontrada | Se usa la lista global (`listaCursos`). Se llama con `tematica="Salud"` (temática que no existe) y `nivel=1`. | El bucle (2) itera, pero la condición (3,4) (coincidencia de temática) es siempre Falsa. El método devuelve `null`. Se comprueba con `assertNull(resultado)`. |
| **1 → 2 (Loop) → 3,4 (T) → 5 (Fin) → 11 → 12 → 14 → 15 → Fin** | **(Camino No Factible)** | N/A | N/A |
| **1 → 2 (Loop) → 3,4 (T) → 5 (Loop) → 6 (F) → 10 → 5 (Fin) → 11 → 12 → 14 → 15 → Fin** | test\_MejorValorado\_TematicaOk\_NivelNoEncontrado | Se usa la lista global (`listaCursos`). Se llama con `tematica="Informatica"` (existe) y `nivel=99` (nivel que no existe). | El bucle (5) itera sobre los cursos de "Informatica", pero la condición (6) (`c.getNivel() == nivel`) es siempre Falsa. El método devuelve `null`. Se comprueba con `assertNull(resultado)`. |
| **1 → 2 (Loop) → 3,4 (T) → 5 (Loop) → 6 (T) → 7 (F) → 9 → 10 → 5 (Fin) → 11 → 12 → 14 → 15 → Fin** | **(Camino No Factible)** | N/A | N/A |
| **1 → 2 (Loop) → 3,4 (T) → 5 (Loop) → 6 (T) → 7 (T) → 8 → 9 → 10 → 5 (Fin)→ 11 → 12 → 14 → 15 → Fin** | test\_MejorValorado\_TematicaOk\_UnCursoOk | Se usa la lista global (`listaCursos`). Se llama con `tematica="Inomatica"` y `nivel=2`. | El bucle (5) itera: `info1` (N1) no cumple (6). `info2` (N2) cumple (6) y (7) (porque `mejor` es `null`), `mejor` se asigna a `info2`. `info3` (N3) no cumple (6). Se ejecuta el `break` (12). El método devuelve `info2`. Se comprueba con `assertEquals(info2, resultado)`. |
